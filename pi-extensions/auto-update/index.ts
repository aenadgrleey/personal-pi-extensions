...
export function buildSelfUpdateCommand(method: InstallMethod, version: string): SelfUpdateCommand | undefined {
	...
	switch (method) {
		...
		case "unknown":
			return undefined;
		case "pnpm":
			return { command: "pnpm", args: ["install", "-g", spec], display: `pnpm install -g ${spec}` };
		case "yarn":
			return { command: "yarn", args: ["global", "add", spec], display: `yarn global add ${spec}` };
		case "bun":
			return { command: "bun", args: ["install", "-g", spec], display: `bun install -g ${spec}` };
		case "npm":
...
export default function (pi: ExtensionAPI) {
	...
	const runUpdate = async (ctx: ExtensionContext) => {
		...
		try {
			const currentResult = await pi.exec("pi", ["--version"], { timeout: 10_000 });
			...
			const updateCommand = buildSelfUpdateCommand(detectInstallMethod(), latestVersion);
			if (!updateCommand) {
				ctx.ui.setStatus("auto-update", "✗ update unavailable");
				ctx.ui.notify("Auto-update unavailable for this pi installation", "error");
				return;
			}

			ctx.ui.setStatus("auto-update", `⏳ updating pi ${currentVersion} → ${latestVersion}…`);

			const updateResult = await pi.exec(updateCommand.command, updateCommand.args, { timeout: 120_000 });

			if (updateResult.code === 0) {
				const verifiedResult = await pi.exec("pi", ["--version"], { timeout: 10_000 });
				const verifiedVersion = extractVersion(verifiedResult.stdout);
				if (verifiedVersion !== latestVersion) {
					ctx.ui.setStatus("auto-update", `✗ update mismatch (${verifiedVersion})`);
					ctx.ui.notify(
				...
				ctx.ui.setStatus("auto-update", `✓ pi updated to ${latestVersion} (restart to use)`);
				ctx.ui.notify(`pi updated to ${latestVersion} — restart to use the new version`, "info");
			} else {
				const output = (updateResult.stdout + updateResult.stderr).trim().slice(0, 200);
