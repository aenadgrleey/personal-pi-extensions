...
function checkCandidates(
  ...
  while (true) {
    ...
    candidates.push({
      location: formatLocation(cwd, rootChecks),
      path: rootChecks,
    });
    candidates.push({
      location: formatLocation(cwd, piChecks),
      path: piChecks,
    });
...
function packageScriptSkipMessage(
  cwd: string,
  command: string,
): string | undefined {
  ...
  if (!existsSync(packageJsonPath))
    return `Skipping ${script}: package.json not found`;

  const raw = readFileSync(packageJsonPath, "utf-8");
  const packageJson = JSON.parse(raw) as { scripts?: Record<string, string> };
  if (!packageJson.scripts?.[script])
    return `Skipping ${script}: package.json script not configured`;
...
async function runChecks(
  ...
  return {
    ...
    results: await Promise.all(
      loaded.checks.map((c) => runCheck(exec, c, loaded.cwd)),
    ),
  };
}
...
function buildCheckMessage(
  source: string,
  results: CheckResult[],
): { content: string; details: CheckDetails } {
...
function isKnownCheckCommand(cwd: string, command: string): boolean {
  ...
  return loaded.checks.some(
    (check) => normalizeCommand(check.command) === normalized,
  );
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer<CheckDetails>(
    "check-result",
    (message, _options, theme) => {
      ...
      if (allPassed) {
        return new Text(
          `${icon} ${theme.fg("success", "all checks pass")}`,
          0,
          0,
        );
      }
    ...
    if (!run) {
      return {
        content: [
          {
            type: "text" as const,
            text: "No checks found in checks.yaml or .pi/checks.yaml",
          },
        ],
        details: undefined,
      };
  ...
  function sendCheckMessage(result: {
    content: [{ type: "text"; text: string }];
    details: CheckDetails | undefined;
  }) {
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Run project checks from checks.yaml or .pi/checks.yaml when you need an explicit verification run",
    promptGuidelines: [
