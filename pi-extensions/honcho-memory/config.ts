...
export const getSessionStrategyLabel = (
...
export const resolveConfig = async (): Promise<HonchoExtensionConfig> => {
  ...
  const enabled =
    enabledEnv !== undefined ? enabledEnv === "true" : Boolean(apiKey);

  const baseURL = process.env.HONCHO_URL || piHost?.endpoint || undefined;
  const workspaceId =
    process.env.HONCHO_WORKSPACE_ID || piHost?.workspace || "pi";
  const userPeerId =
  ...
  const exportConversation =
    process.env.HONCHO_EXPORT_CONVERSATION === "true" ||
    (process.env.HONCHO_EXPORT_CONVERSATION === undefined &&
      piHost?.exportConversation === true);
