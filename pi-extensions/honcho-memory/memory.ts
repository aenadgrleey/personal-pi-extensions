...
export const buildUserProfileText = (
  userProfile: string | null,
): string | null => buildSection(USER_PROFILE_LABEL, userProfile);

export const buildProjectSummaryText = (
  projectSummary: string | null,
): string | null => buildSection(PROJECT_SUMMARY_LABEL, projectSummary);
...
export const getCachedMemory = (): string | null =>
  buildCombinedMemoryText(cachedMemory);
...
const CREDENTIAL_PATTERNS = [
  ...
  /\b(?:sk|hch|rk)-[a-z0-9_-]{8,}\b/i,
  /\bBearer\s+[a-z0-9._-]{8,}\b/i,
  /\b(?:access|refresh)[_-]?token\s*[:=]/i,
  /\bapi[_-]?key\s*[:=]/i,
  /\b[a-z0-9_-]{12,}\.[a-z0-9_-]{12,}\.[a-z0-9_-]{12,}\b/i,
];

/** Do not send messages with credential-shaped content to a remote service. */
export const isSafeConversationText = (text: string): boolean =>
...
export const saveMessages = (
  ...
  const pairs = extractConversationalPairs(
    messages,
    handles.config.maxMessageLength,
  );
  if (pairs.length === 0) {
