...
const tryGitBranch = async (
  pi: ExtensionAPI,
  cwd: string,
): Promise<string | null> => {
  const branchResult = await execGit(pi, cwd, [
    "rev-parse",
    "--abbrev-ref",
    "HEAD",
  ]);
...
const deriveRepoScopedKey = async (
  pi: ExtensionAPI,
  cwd: string,
): Promise<string> => {
