...
import { isSafeConversationText } from "./memory.js";
...
const formatResults = (
  ...
  results
    .map(
      (mem, idx) =>
        `${idx + 1}. [${mem.peerId}] ${mem.content.slice(0, previewLength)}`,
    )
    .join("\n\n");
...
export const registerTools = (pi: ExtensionAPI): void => {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use honcho_chat for reasoning over memory, not simple lookup.",
    ],
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    description:
      "Write an explicit durable fact, preference, or decision to persistent memory",
    promptSnippet:
      "Save a durable fact, preference, or decision to persistent memory",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      if (!isSafeConversationText(params.content)) {
        throw new Error(
          "Refusing to save credential-shaped content to persistent memory.",
        );
      }
      const handles = ensureConnected();
      ...
      return {
        content: [
          { type: "text" as const, text: `Remembered: ${params.content}` },
        ],
        details: {},
      };
    },
