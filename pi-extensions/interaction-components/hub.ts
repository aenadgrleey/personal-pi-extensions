...
export interface InteractionProvider {
	...
	present<T>(pending: PresentedInteraction<T>, ctx: ExtensionContext): Promise<(() => void | Promise<void>) | void>;
	onResolved?<T>(record: PendingInteractionRecord, resolution: InteractionResolution<T>): Promise<void>;
...
function isProviderCancellation(interaction: SharedInteraction, value: unknown): boolean {
...
export class InteractionHub implements InteractionBridge {
	...
	private async present<T>(ctx: ExtensionContext, interaction: SharedInteraction): Promise<T> {
		...
		const activeProviders = this.providers.filter((provider) => provider.isActive(ctx));
		if (activeProviders.length === 0) {
			throw new Error(`No active interaction providers for ${interaction.kind}`);
		}

		const resolution = await new Promise<InteractionResolution<T>>((resolve, reject) => {
	...
	private resolve<T>(interactionId: string, resolution: InteractionResolution<T>): void {
		const pending = this.pending.get(interactionId) as PendingRuntime<T> | undefined;
		...
		if (isProviderCancellation(pending.record.interaction, resolution.value)) {
			...
			if (pending.cancelledProviderIds.size < pending.activeProviderIds.length) return;
		}
		pending.settled = true;
		pending.resolvePromise(resolution);
	}

	async presentAsk(ctx: ExtensionContext, questions: BridgeAskQuestion[]): Promise<BridgeAskResult> {
	...
	async presentPlanDecision(
		...
		return this.present<BridgePlanDecisionResult>(ctx, { kind: "plan", params });
	}
	...
	async presentReview(
		...
		return this.present<BridgeReviewPromptResult | undefined>(ctx, { kind: "review", params });
	}
