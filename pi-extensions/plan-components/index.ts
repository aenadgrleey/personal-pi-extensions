...
import { Container, Spacer, Text, truncateToWidth, visibleWidth } from "../deps.js";
import {
	buildPlanText,
	savePlanToFile,
	type PlanPhase,
...
export { loadPlanFile, buildPlanFilePath, buildPlanSaveDir, buildPlanText, savePlanToFile } from "./utils.js";
...
export async function showLocalPlanDecision(
	...
	try {
		...
		const idx = SELECT_LABELS.indexOf(selected as (typeof SELECT_LABELS)[number]);
		const action = (idx >= 0 ? SELECT_ACTIONS[idx] : "discarded") as PlanDecisionResult["action"];
...
export async function showPlanPreview(
	...
	const filePath = decision.action === "accepted" || decision.action === "saved"
	...
	properties: {
		...
		phases: {
			...
			items: {
				type: "object",
				properties: {
					name: { type: "string", description: "Phase name (e.g. 'Schema & Resolvers')" },
					steps: {
						type: "array",
		...
		context: { type: "string", description: "Optional background context shown above the phases" },
	},
...
export const planPreviewTool = {
	...
	renderCall(args: PlanPreviewParams, theme: Theme) {
		const title = typeof args.title === "string" && args.title.trim() ? args.title : "Untitled Plan";
		...
		for (const phase of phases) {
			...
			for (const step of phase.steps ?? []) {
				children.push(new Text(theme.fg("text", `  ${globalStep}. ${step}`), 0, 0));
				globalStep++;
			}
		}

		children.push(new Spacer(1));
		children.push(new Text(theme.fg("dim", `${phases.length} phase(s), ${totalSteps} step(s)`), 0, 0));

		const container = new Container();
		for (const child of children.filter((child): child is Text | Spacer => child !== undefined)) {
	...
	renderResult(
		result: { details?: PlanPreviewResult; content: Array<{ type: string; text?: string }> },
		...
		switch (details.action) {
			case "accepted":
				return new Text(
					formatResultLabel("✓", "Accepted", 1) + theme.fg("dim", ` ${details.planPath}`),
					0,
					0,
				);
			case "saved":
			...
			case "refined":
				return new Text(formatResultLabel("✏️", "Refinement requested", 1), 0, 0);
			case "discarded":
