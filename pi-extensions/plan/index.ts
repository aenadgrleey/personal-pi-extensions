...
async function listPlanFiles(cwd: string): Promise<string[]> {
	...
	try {
		...
		return files.filter((f) => f.endsWith("-plan.yaml")).sort().reverse();
	} catch {
		return [];
	}
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("plan", {
		description: "Manage saved plans (list, view). Usage: /plan [list|view <path>]",
		async handler(args, ctx) {
				...
				const items = await Promise.all(
					files.map(async (f) => {
						...
						try {
							...
							const stepCount = plan.phases.reduce((s, p) => s + p.steps.length, 0);
			...
			ctx.ui.notify(`Unknown subcommand "${sub}". Use: /plan [list|view <path>]`, "error");
		},
	});
}
