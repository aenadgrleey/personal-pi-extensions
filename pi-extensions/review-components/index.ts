...
function makeResult(
  review: ReviewPromptParams,
  result: ChainReviewResult,
): ReviewPromptResult {
  return { ...result, review };
}

function cancelledResult(review: ReviewPromptParams): {
  content: { type: "text"; text: string }[];
  details: ReviewToolDetails;
} {
  return {
...
const ReviewPromptSchema = {
  ...
  properties: {
    decision: {
      type: "string",
      description: "Short label for the decision to review",
    },
    summary: { type: "string", description: "What the user should review" },
    context: {
      type: "string",
      description: "Optional background context shown above the summary",
    },
  },
...
export const reviewTool = {
  ...
  renderCall(args: ReviewPromptParams, theme: Theme) {
    ...
    container.addChild(
      new Text(theme.fg("accent", `⚠️  ${args.decision}`), 0, 0),
    );
    ...
    container.addChild(new Spacer(1));
    container.addChild(
      new Text(theme.fg("dim", "Keep as is or revise this decision."), 0, 0),
    );
    return container;
  },
