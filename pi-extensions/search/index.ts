...
type ToolName = "grepsearch" | "websearch" | "codesearch" | "context7" | "web_fetch";
...
const grepsearchParamsSchema = {
	...
	properties: {
		...
		language: {
			type: "string",
			description: "Filter by language: TypeScript, TSX, Python, Go, Rust, etc.",
		},
...
const websearchParamsSchema = {
	...
	properties: {
		...
		type: {
			type: "string",
			description: '"auto" (default), "neural" (semantic), or "keyword" (exact match)',
		},
	},
	required: ["query"],
	additionalProperties: false,
...
const context7ParamsSchema = {
	...
	properties: {
		...
		offset: {
			type: "number",
			description: "Character offset to start reading from (for paginating long docs)",
			minimum: 0,
		},
	},
	additionalProperties: false,
...
async function callMcpTool(
	...
	try {
		...
		if (!response.ok) {
			throw new Error(`${toolName} backend returned ${response.status}: ${response.statusText}`);
		}
...
export function buildGrepMcpArguments(params: GrepsearchParams): Record<string, unknown> {
...
function parseGrepMcpBlock(block: string): ParsedGrepBlock | undefined {
	...
	const repo = lines.find((line) => line.startsWith("Repository:"))?.slice("Repository:".length).trim();
	const path = lines.find((line) => line.startsWith("Path:"))?.slice("Path:".length).trim();
	if (!repo || !path) return undefined;

	const url = lines.find((line) => line.startsWith("URL:"))?.slice("URL:".length).trim();
	const license = lines.find((line) => line.startsWith("License:"))?.slice("License:".length).trim();
	const snippetsIndex = lines.findIndex((line) => line.trim() === "Snippets:");
	const snippet = (snippetsIndex === -1 ? lines : lines.slice(snippetsIndex + 1)).join("\n").trim();
...
export function formatGrepMcpBlocks(blocks: string[], limit: number): string {
	...
	const formatted = shown.map((block, index) => {
		...
		const lines = [`## ${index + 1}. ${parsed.repo}`, `**File**: ${parsed.path}`];
...
function context7HttpError(status: number, operation: string, libraryId?: string): TextToolResult {
	if (status === 401) {
		return toolResult(
	...
	return toolResult(`Error: Context7 API returned ${status}`, { operation, error: `http_${status}` });
}
...
Examples:
  ...
  grepsearch({ query: "export async function POST", path: "route.ts" })`,
			promptSnippet: "Search real-world code examples from GitHub repos via Grep MCP.",
			parameters: grepsearchParamsSchema,
			async execute(_toolCallId, params: GrepsearchParams, signal): Promise<TextToolResult> {
				const { query, language, limit = 10 } = params;

				if (!query?.trim()) {
					return toolResult("Error: query is required", { error: "query required" });
				}
				...
				try {
					...
					if (blocks.length === 0) {
						return toolResult(`No results found for: ${query}${language ? ` (${language})` : ""}`, {
							query,
							results: 0,
		...
		pi.registerTool({
			...
			async execute(
				...
				try {
					...
					const message = errorMessage(error);
					return toolResult(`Code search failed: ${message}`, { backend: "web_search_exa" });
				}
			},
		});
	}
			...
			async execute(
				...
				if (operation === "resolve") {
					...
					try {
						...
						if (!response.ok) return context7HttpError(response.status, "resolve");
						...
						return {
							content: [
								{ type: "text" as const, text: `Error resolving library: ${message}` },
							],
				...
				try {
					...
					if (!response.ok) return context7HttpError(response.status, "query", libraryId);
					...
					return {
						content: [
							{ type: "text" as const, text: `Error querying documentation: ${message}` },
						],
...
Example:
  ...
  web_fetch({ url: "https://example.com/article", offset: 20000 })`,
			...
			async execute(
				...
				if (!params.url?.trim()) {
					return toolResult("Error: url is required", { error: "url required" });
				}
				...
				try {
					...
					return toolResult(paginateText(blocks.join("\n"), params.offset ?? 0, MAX_PAGE_CHARS), {});
				} catch (error: unknown) {
					const message = errorMessage(error);
